# Compute sequence of 2 events – OMOP
...
- cohort:
  ...
  a table with the cohort of interest; should minimally include:
...
- grouped_list:
  ...
  list of columns in the data that should be used to group the results

- site_col:

  the name of the column with site name(s)

- user_cutoff:

  a user specified threshold against which to measure the number of days
...
- n_event_b:
  ...
  the number of times Event B should occur before esrablishing the
  occurrence date

- patient_level_tbl:

  a logical to indicate whether an intermediate table with patient level
  results should be output. if TRUE, a dataframe with one row per
  patient will be returned in addition to aggregate output
...
- event_csv:
  ...
  a csv file with the definitions for each event with the columns
...
an aggregated dataframe outlining the number of patients that had the
two events occur in X number of days
